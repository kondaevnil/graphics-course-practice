...
void main()
{
    ...
    if (shadow_factor < delta) {
        ...
        shadow_factor = (shadow_factor - delta) / (1 - delta);
    }
...
void main()
{
    ...
    out_coord = vec4(z, z * z + 0.25 * (dFdx(z) * dFdx(z) + dFdy(z) * dFdy(z)), 0.0, 0.0);
}
)";
...
std::pair<glm::vec3, glm::vec3> scene_bb(obj_data &scene) {
    auto [x, y, z] = scene.vertices[0].position;
    glm::vec3 min = {x, y, z};
    glm::vec3 max = {x, y, z};
    for (auto vert: scene.vertices) {
        auto [xp, yp, zp] = vert.position;
        min.x = std::min(xp, min.x);
        min.y = std::min(yp, min.y);
        min.z = std::min(zp, min.z);
        max.x = std::max(xp, max.x);
        max.y = std::max(yp, max.y);
        max.z = std::max(zp, max.z);
    }
...
int main() try
{
    ...
    glFramebufferTexture(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, shadow_rbo, 0);
    ...
    while (running)
    {
        ...
        float x_len = std::abs(glm::dot(bound_box[0] - center, light_x));
        float y_len = std::abs(glm::dot(bound_box[0] - center, light_y));
        float z_len = std::abs(glm::dot(bound_box[0] - center, light_z));

        for (auto v: bound_box) {
            x_len = std::max(x_len, std::abs(glm::dot(v - center, light_x)));
            y_len = std::max(y_len, std::abs(glm::dot(v - center, light_y)));
            z_len = std::max(z_len, std::abs(glm::dot(v - center, light_z)));
        }

        glm::mat4 transform = glm::mat4(1.f);
        for (int i = 0; i < 3; ++i)
        {
            transform[i][0] = x_len * light_x[i];
            transform[i][1] = y_len * light_y[i];
            transform[i][2] = z_len * light_z[i];
